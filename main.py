...
class CashCalculator(Calculator):
    ...
    def get_today_cash_remained(self, currency,
        ...
        # В присвоении в currency_type значения currency нет никакого смысла
        # Везде ниже вместо в условиях if следовало бы проверять значение currency,
        # А в currency_type присваивать уже отображаемое имя типа валюты
        currency_type = currency
        cash_remained = self.limit - self.get_today_stats()
        
        # Не обязательно к исправлению, но стоит иметь ввиду:
        # Для трех валют предоженный ниже подход перевода и получения
        # отображаемого имени приемлем, но при увеличении количества валют
        # данный подход выглядел бы слишком громоздко
        # Можно обойтись без if конструкции, добавив словарь для перевода
        # курсов валют и получения отображаемого типа валюты:
        #currency_dict = {'usd':[USD_RATE,'USD'],
        #                 'eur':[EURO_RATE, 'Euro'],
        #                 'rub':[1.0,'руб']
        #                }
        # Тогда весь блок if`ов можно заменить следущими командами
        # currency_type = currency_dict[currency][1]
        # cash_remained /= currency_dict[currency][0]
        ...
        if currency == 'usd':
            ...
            # иначе просто убрать эту строку
            cash_remained == 1.00
            currency_type = 'руб'
        # Возможно, следует описать обработку ситуации, когда был передан
        # некорректный параметр currency. Сейчас программа выведет некоторую
        # сумму без указания единиц измерения
        if cash_remained > 0:
            return (
